...
public class UnsignedTransactionBuilderImpl implements UnsignedTransactionBuilder {
  ...
  @Override
  public UnsignedTransactionBuilder withInputs(String... inputBoxIds) {
  ...
  @Override
  public UnsignedTransactionBuilder withCandidates(OutBox... candidates) {
    _outputCandidates = new ArrayList<>();
    ErgoBox[] boxes = Stream.of(candidates).<ErgoBox>map(c -> ((OutBoxImpl)c).getErgoBox()).toArray(n -> new ErgoBox[n]);
