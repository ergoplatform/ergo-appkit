...
#native-image --no-server -cp target/scala-2.12/ergo-appkit-3.0.0.jar \
...
#  --initialize-at-run-time=org.ergoplatform.appkit.impl.SignedTransactionImpl \
#  org.ergoplatform.appkit.ni.Prove prove
...
native-image --no-server -cp target/scala-2.12/ergo-appkit-3.0.0.jar \
  ...
  -H:+AllowVMInspection -H:-RuntimeAssertions --enable-url-protocols=http,https \
  --allow-incomplete-classpath \
  org.ergoplatform.appkit.PrepareBox preparebox
