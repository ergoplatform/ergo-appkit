...
import org.ergoplatform.example.util.{FileMockedErgoClient, JFunction}

object RunMockedScala extends App {
  import org.ergoplatform.example.MockData._
  val res = new FileMockedErgoClient(infoFile, lastHeadersFile, boxFile).execute(JFunction.instance({ ctx =>
