...
class Eip29AttachmentSpec extends PropSpec {
  ...
  property("build plain text") {
    ...
    val backFromValue: Eip29Attachment = Eip29GenericAttachment.createFromErgoValue(ergoValue)
    Eip29Attachment.Type.PLAIN_TEXT shouldBe backFromValue.getType
    backFromValue.isInstanceOf[Eip29PlainTextAttachment] shouldBe true
    attachment.getText shouldBe backFromValue.asInstanceOf[Eip29PlainTextAttachment].getText
    ...
    attachment.getText shouldBe backFromHex.asInstanceOf[Eip29PlainTextAttachment].getText
  ...
  property("multi attachment test") {
    ...
    textAttachmentContent shouldBe multiAttachment.getAttachment(0).asInstanceOf[Eip29PlainTextAttachment].getText
    ...
    val backFromValue = Eip29GenericAttachment.createFromErgoValue(ergoValue)
    backFromValue.getType shouldBe Eip29Attachment.Type.MULTI_ATTACHMENT
    backFromValue.isInstanceOf[Eip29MultiAttachment] shouldBe true
    val multiBackFromValue = backFromValue.asInstanceOf[Eip29MultiAttachment]
    multiAttachment.getAttachmentCount shouldBe multiBackFromValue.getAttachmentCount
    textAttachmentContent shouldBe multiBackFromValue.getAttachment(0).asInstanceOf[Eip29PlainTextAttachment].getText
  }
}
